...
% \titulo        = titulo na CAPA e FOLHA DE ROSTO (portugues)
% \tituloresumo  = titulo que aparece no RESUMO (portugues, igual ao de cima)
% \titleabstract = titulo no ABSTRACT (ingles)
% \tituloadic    = titulo na FOLHA DE ROSTO ADICIONAL (ingles)
\titulo{Avalia\c{c}\~ao da Experi\^encia do Usu\'ario do Aplicativo Trilha das \'Arvores usando m\'etricas heur\'isticas e com estudo com usu\'arios finais}
\tituloresumo{Avalia\c{c}\~ao da Experi\^encia do Usu\'ario do Aplicativo Trilha das \'Arvores usando m\'etricas heur\'isticas e com estudo com usu\'arios finais}
% >>> AJUSTE a traducao do titulo em ingles abaixo se quiser <<<
\titleabstract{User Experience Evaluation of the Trilha das \'Arvores Application using heuristic metrics and a study with end users}
\tituloadic{User Experience Evaluation of the Trilha das \'Arvores Application using heuristic metrics and a study with end users}
...
\autor{João Victor de Almeida}          % Ex.: Maria Aparecida de Souza
\autorficha{Almeida, João}        % Formato ficha: Souza, Maria Aparecida de
\autorabr{Almeida, J. V.}            % Formato abreviado: SOUZA, M. A.

% O Codigo Cutter e fornecido pela Biblioteca (ICMC) junto com a ficha
% catalografica. Enquanto nao tiver, pode deixar como esta.
\cutter{X000x}
...
\local{S\~ao Carlos}
\data{2026}   % >>> AJUSTE o ano de deposito/defesa <<<
% Para o idioma português:
% >>> PREENCHA com os dados do(a) seu(sua) orientador(a) <<<
% Use "Orientador:" ou "Orientadora:" conforme o caso.
\renewcommand{\orientadorname}{Orientadora:}
\orientador{Prof. Dra do Orientador}
\orientadoradic{Advisor: Prof. Dr. Nome do Orientador}
\orientadorcorpoficha{orientador Nome do Orientador}
\orientadorficha{Sobrenome, Nome, orient}
